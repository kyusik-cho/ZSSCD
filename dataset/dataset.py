...
class CDDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_t0, img_t1, mask, ref_rgb_path, test_rgb_path = self.get_raw(index) # shape: [6,512,512], [512,512]
        img_t0 = np.array(img_t0)
        img_t1 = np.array(img_t1)
        mask = np.array(mask)
        return img_t0, img_t1, mask, ref_rgb_path, test_rgb_path#.split('/')[-1]
