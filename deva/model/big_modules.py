...
class PixelEncoder(nn.Module):    
    def __init__(self, pix_feat_dim: int, adain_layer):
        super().__init__()
        self.pix_feat_dim = pix_feat_dim

        self.adain_layer = adain_layer
        network = resnet.resnet50(pretrained=True, adain_layer=self.adain_layer)
        self.conv1 = network.conv1
        self.instance_norm = network.instance_norm
    ...
    def init_styles(self):
        for m in self.modules():
            if isinstance(m, AdaptiveInstanceNormalization):
                m.init_style()

    def set_style_update(self, need_style_update):
        for m in self.modules():
            if isinstance(m, AdaptiveInstanceNormalization):
                m.need_style_update = need_style_update

                # m.need_style_update = True # ablation: always true
...
class MaskEncoder(nn.Module):
    ...
    def forward(self,
        ...
        try:
            ...
            # new_sensory = new_sensory.cpu()
            # del sensory, all_g, ms_features
            # torch.cuda.empty_cache()
            
            # g = torch.cat(cpu_g, dim=1).cuda()
            # new_sensory = new_sensory.cuda()
