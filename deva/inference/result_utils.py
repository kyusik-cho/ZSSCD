...
class ResultSaver:
    def __init__(self,
        ...
        if self.need_remapping:
            self.id2rgb_converter = ID2RGBConverter(random=False)
...
def save_result(queue: Queue):
    while True:
        ...
        if save_the_mask:
            if saver.object_manager.use_long_id: # False -> True
                out_mask = mask.numpy().astype(np.uint32)
                rgb_mask = np.zeros((*out_mask.shape[-2:], 3), dtype=np.uint8)
                for id in all_obj_ids:
                    colored_mask = saver.id2rgb_converter.convert(id)[1]
            ...
            if saver.visualize and saver.object_manager.use_long_id:
                if image_np is None:
                    if path_to_image is not None:
                        image_np = np.array(Image.open(path_to_image).resize(out_mask.shape[:2][::-1], Image.BICUBIC))
                    else:
