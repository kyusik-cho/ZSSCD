...
class DEVAInferenceCore:
    ...
    def incorporate_detection(self,
        ...
        ms_features = self.image_feature_store.get_ms_features(image_ti, image) # 40 MB
    ...
    def step(self,
        ...
        self.curr_ti += 1 if update_sensory else 0
        ...
        is_mem_frame = ((self.curr_ti - self.last_mem_ti >= self.mem_every) or (mask is not None)) and (not end) and update_sensory
        ...
        if need_segment:
            pred_prob_with_bg = self._segment(key, selection, ms_features, update_sensory=update_sensory * (not end))
