...
class ObjectManager:
    ...
    def __init__(self):
        ...
        self.use_long_id = True # False -> True 
    ...
    def add_new_objects(
        ...
        for obj in objects:
            ...
            while (new_obj.id in self.all_historical_object_ids) or (self.use_long_id
                                                                     and new_obj.id < 256):
                count += 1
                if count + 255 in self.all_historical_object_ids:
                    continue
                ...
                # print(self.all_historical_object_ids)
            ...
            # self.all_historical_object_ids.add(count)
    ...
    # def purge_small_objects(self, counts: torch.Tensor,
    #                            size_threshold: int) -> (bool, List[int], List[int]):
    #     # remove tmp ids of objects that are smaller than threshold
    #     obj_id_to_be_deleted = []
    #     tmp_id_to_be_deleted = []
    #     tmp_id_to_keep = []
    #     obj_id_to_keep = []

    #     # bool_keep_idx = counts > size_threshold
    #     # tmp_id_to_keep = torch.where(bool_keep_idx)[0] + 1 # bool_keep_idx == True

    #     for tmp, count in enumerate(counts): # idx starts from 0, tracking starts from 1
    #         if count > size_threshold:
    #             tmp_id_to_keep.append(tmp + 1)
    #             obj_id_to_keep.append(self.tmp_id_to_obj[tmp + 1].id)

    #         else: 
    #             tmp_id_to_be_deleted.append(tmp + 1)
    #             obj_id_to_be_deleted.append(self.tmp_id_to_obj[tmp + 1].id)
    ...
    #     #return purge_activated, tmp_id_to_keep, obj_id_to_keep
    #     return purge_activated, tmp_id_to_keep, obj_id_to_keep
    #     # return purge_activated, tmp_id_to_keep, tmp_id_to_be_deleted, obj_id_to_keep